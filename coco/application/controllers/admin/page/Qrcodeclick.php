 ...
 */
class Qrcodeclick extends CB_Controller
{
	 ...
	 * 페이지 이동시 필요한 정보입니다
	 */
	public $pagedir = 'page/Qrcodeclick';

	/**
	 * 모델을 로딩합니다
	 */
	protected $models = array('Qrcodeclick_log');

	/**
	 * 이 컨트롤러의 메인 모델 이름입니다
	 */
	protected $modelname = 'Qrcodeclick_log_model';
	...
	{
		// 이벤트 라이브러리를 로딩합니다
		$eventname = 'event_admin_page_Qrcodeclick_index';
		 ...
		 */
		$this->{$this->modelname}->allow_search_field = array('qcl_id', 'qr_id', 'qrg_name', 'qrcode.mem_id', 'Qrcodeclick_log.mem_id', 'qcl_datetime', 'qcl_ip'); // 검색이 가능한 필드
		$this->{$this->modelname}->search_field_equal = array('qcl_id', 'qr_id', 'qrcode.mem_id', 'Qrcodeclick_log.mem_id'); // 검색중 like 가 아닌 = 검색을 하는 필드
		$this->{$this->modelname}->allow_order_field = array('qcl_id'); // 정렬이 가능한 필드
		...
		if (element('list', $result)) {
			foreach (element('list', $result) as $key => $val) {
				...
				if (element('qcl_useragent', $val)) {
					$userAgent = get_useragent_info(element('qcl_useragent', $val));
		 ...
		 */
		$search_option = array('qcl_datetime' => '날짜', 'qcl_ip' => 'IP');
	...
	{
		// 이벤트 라이브러리를 로딩합니다
		$eventname = 'event_admin_page_Qrcodeclick_graph';
		$this->load->event($eventname);
		...
		$result = $this->{$this->modelname}->get_Qrcodeclick_count($datetype, $start_date, $end_date, $orderby);
		$sum_count = 0;
		$arr = array();
		$max = 0;
		...
		if ($export === 'excel') {
			...
			header('Content-Disposition: attachment; filename=배너클릭_' . cdate('Y_m_d') . '.xls');
	...
	{
		// 이벤트 라이브러리를 로딩합니다
		$eventname = 'event_admin_page_Qrcodeclick_listdelete';
		$this->load->event($eventname);
	...
	{
		// 이벤트 라이브러리를 로딩합니다
		$eventname = 'event_admin_page_Qrcodeclick_cleanlog';
		$this->load->event($eventname);
		...
		if ($this->form_validation->run() === false) {
			...
			if ($this->input->post('criterion') && $this->input->post('day')) {
				$deletewhere = array(
					'qcl_datetime <=' => $this->input->post('criterion'),
				);
				$this->Qrcodeclick_log_model->delete_where($deletewhere);
				$view['view']['alert_message'] = '총 ' . number_format($this->input->post('log_count')) . ' 건의 ' . $this->input->post('day') . '일 이상된 배너클릭로그가 모두 삭제되었습니다';
			} else {
				$criterion = cdate('Y-m-d H:i:s', ctimestamp() - $this->input->post('day') * 24 * 60 * 60);
				$countwhere = array(
					'qcl_datetime <=' => $criterion,
				);
				$log_count = $this->Qrcodeclick_log_model->count_by($countwhere);
				$view['view']['criterion'] = $criterion;
				$view['view']['day'] = $this->input->post('day');
				$view['view']['log_count'] = $log_count;
				if ($log_count > 0) {
					$view['view']['msg'] = '총 ' . number_format($log_count) . ' 건의 ' . $this->input->post('day') . '일 이상된 배너클릭로그가 발견되었습니다. 이를 모두 삭제하시겠습니까?';
				} else {
					$view['view']['alert_message'] = $this->input->post('day') . '일 이상된 배너클릭로그가 발견되지 않았습니다';
				}
			}
		}
