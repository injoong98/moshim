   <!--Page Title-->
   <section class="page-title centred background-cancel" style="background-image: url(<?php echo moshim_url('momo') ?>/assets/moshim/images/cd/cover_01.jpg);">
       <div class="container">
           <div class="content-box">
               <!-- <h1>CD</h1> -->
               <!-- <ul class="bread-crumb clearfix">
                   <li><a href="index.html">Home</a></li>
                   <li>About</li>
               </ul> -->
           </div>
       </div>
   </section>
   <!--End Page Title-->

   <!-- todo
    
    1. 색상 변경
    2. 노래 넣기
    3. 가사 해설 넣기
     -->
   <!-- our-teachers -->
   <section class="our-teachers style-three about-page sec-pad centred">
       <div class="container">
           <div class="sec-title">
               <h5>참어머님 생애동요 CD</h5>
               <h1> 참어머님의 생애와 심정이 담긴 효정의 노래
                   <br />(7~10세)
               </h1>
           </div>
           <!-- <div class="inner-content"> -->
           <div class="row">
               <div class="col-lg-4 col-md-6 col-sm-12 block-column" style="padding:25px 30px;">
                   <div class="teachers-block-two wow fadeInUp" data-wow-delay="00ms" data-wow-duration="1500ms">
                       <div class="inner-box">
                           <figure class="image-box"><a href="#"><img src="<?php echo moshim_url('momo') ?>/assets/moshim/images/cd/07.jpg" alt=""></a></figure>
                           <div class="lower-content">
                               <h3><a class="cursor_pointer_on" onclick="openWin_cd('1')" target="_blank">달래강 노래</a></h3>
                               <span class="designation"><br /></span>
                               <ul class="social-list">
                                   <li><a class="play_button"><i class="track-01 fas fa-play"></i></a></li>
                                   <li><a class="play_button"><i class="track-01 fas fa-pause"></i></a></li>
                                   <li><a class="cursor_pointer_on" onclick="openWin_cd('1')" target="_blank"><i class="far fa-file-alt"></i></a></li>
                               </ul>
                           </div>
                       </div>
                   </div>
               </div>
               <div class="col-lg-4 col-md-6 col-sm-12 block-column" style="padding:25px 30px;">
                   <div class="teachers-block-two wow fadeInUp" data-wow-delay="300ms" data-wow-duration="1500ms">
                       <div class="inner-box">
                           <figure class="image-box"><a href="#"><img src="<?php echo moshim_url('momo') ?>/assets/moshim/images/cd/09.jpg" alt=""></a></figure>
                           <div class="lower-content">
                               <h3><a onclick="openWin_cd('2')" target="_blank">아름다운 이 세상</a></h3>
                               <span class="designation"><br /></span>
                               <ul class="social-list ">
                                   <li><a class="play_button"><i class="track-02 fas fa-play"></i></a></li>
                                   <li><a class="play_button"><i class="track-02 fas fa-pause"></i></a></li>
                                   <li><a class="cursor_pointer_on" onclick="openWin_cd('2')" target="_blank"><i class="far fa-file-alt"></i></a></li>
                               </ul>
                           </div>
                       </div>
                   </div>
               </div>
               <div class="col-lg-4 col-md-6 col-sm-12 block-column" style="padding:25px 30px;">
                   <div class="teachers-block-two wow fadeInUp" data-wow-delay="600ms" data-wow-duration="1500ms">
                       <div class="inner-box">
                           <figure class="image-box"><a href="#"><img src="<?php echo moshim_url('momo') ?>/assets/moshim/images/cd/11.jpg" alt=""></a></figure>
                           <div class="lower-content">
                               <h3><a onclick="openWin_cd('3')" target="_blank">하나님의 나의 부모님</a></h3>
                               <span class="designation"><br /></span>
                               <ul class="social-list">
                                   <li><a class="play_button"><i class="track-03 fas fa-play"></i></a></li>
                                   <li><a class="play_button"><i class="track-03 fas fa-pause"></i></a></li>
                                   <li><a class="cursor_pointer_on" onclick="openWin_cd('3')" target="_blank"><i class="far fa-file-alt"></i></a></li>
                               </ul>
                           </div>
                       </div>
                   </div>
               </div>
               <div class="col-lg-4 col-md-6 col-sm-12 block-column" style="padding:25px 30px;">
                   <div class="teachers-block-two wow fadeInUp" data-wow-delay="00ms" data-wow-duration="1500ms">
                       <div class="inner-box">
                           <figure class="image-box"><a href="#"><img src="<?php echo moshim_url('momo') ?>/assets/moshim/images/cd/13.jpg" alt=""></a></figure>
                           <div class="lower-content">
                               <h3><a class="cursor_pointer_on" onclick="openWin_cd('4')" target="_blank">수선화 사랑</a></h3>
                               <span class="designation"><br /></span>
                               <ul class="social-list">
                                   <li><a class="play_button"><i class="track-04 fas fa-play"></i></a></li>
                                   <li><a class="play_button"><i class="track-04 fas fa-pause"></i></a></li>
                                   <li><a class="cursor_pointer_on" onclick="openWin_cd('4')" target="_blank"><i class="far fa-file-alt"></i></a></li>
                               </ul>
                           </div>
                       </div>
                   </div>
               </div>
               <div class="col-lg-4 col-md-6 col-sm-12 block-column" style="padding:25px 30px;">
                   <div class="teachers-block-two wow fadeInUp" data-wow-delay="300ms" data-wow-duration="1500ms">
                       <div class="inner-box">
                           <figure class="image-box"><a href="#"><img src="<?php echo moshim_url('momo') ?>/assets/moshim/images/cd/15.jpg" alt=""></a></figure>
                           <div class="lower-content">
                               <h3><a class="cursor_pointer_on" onclick="openWin_cd('5')" target="_blank">하늘 신부</a></h3>
                               <span class="designation"><br /></span>
                               <ul class="social-list">
                                   <li><a class="play_button"><i class="track-05 fas fa-play"></i></a></li>
                                   <li><a class="play_button"><i class="track-05 fas fa-pause"></i></a></li>
                                   <li><a class="cursor_pointer_on" onclick="openWin_cd('5')" target="_blank"><i class="far fa-file-alt"></i></a></li>
                               </ul>
                           </div>
                       </div>
                   </div>
               </div>
               <div class="col-lg-4 col-md-6 col-sm-12 block-column" style="padding:25px 30px;">
                   <div class="teachers-block-two wow fadeInUp" data-wow-delay="600ms" data-wow-duration="1500ms">
                       <div class="inner-box">
                           <figure class="image-box"><a href="#"><img src="<?php echo moshim_url('momo') ?>/assets/moshim/images/cd/17.jpg" alt=""></a></figure>
                           <div class="lower-content">
                               <h3><a class="cursor_pointer_on" onclick="openWin_cd('6')" target="_blank">산 넘고 바다 건너</a></h3>
                               <span class="designation"><br /></span>
                               <ul class="social-list">
                                   <li><a class="play_button"><i class="track-06 fas fa-play"></i></a></li>
                                   <li><a class="play_button"><i class="track-06 fas fa-pause"></i></a></li>
                                   <li><a class="cursor_pointer_on" onclick="openWin_cd('6')" target="_blank"><i class="far fa-file-alt"></i></a></li>
                               </ul>
                           </div>
                       </div>
                   </div>
               </div>
           </div>
           <div class="row">
               <div class="col-lg-4 col-md-6 col-sm-12 block-column" style="padding:25px 30px;">
                   <div class="teachers-block-two wow fadeInUp" data-wow-delay="00ms" data-wow-duration="1500ms">
                       <div class="inner-box">
                           <figure class="image-box"><a href="#"><img src="<?php echo moshim_url('momo') ?>/assets/moshim/images/cd/19.jpg" alt=""></a></figure>
                           <div class="lower-content">
                               <h3><a class="cursor_pointer_on" onclick="openWin_cd('7')" target="_blank">효정의 어머니</a></h3>
                               <span class="designation"><br /></span>
                               <ul class="social-list">
                                   <li><a class="play_button"><i class="track-07 fas fa-play"></i></a></li>
                                   <li><a class="play_button"><i class="track-07 fas fa-pause"></i></a></li>
                                   <li><a class="cursor_pointer_on" onclick="openWin_cd('7')" target="_blank"><i class="far fa-file-alt"></i></a></li>
                               </ul>
                           </div>
                       </div>
                   </div>
               </div>
               <div class="col-lg-4 col-md-6 col-sm-12 block-column" style="padding:25px 30px;">
                   <div class="teachers-block-two wow fadeInUp" data-wow-delay="300ms" data-wow-duration="1500ms">
                       <div class="inner-box">
                           <figure class="image-box"><a href="#"><img src="<?php echo moshim_url('momo') ?>/assets/moshim/images/cd/21.jpg" alt=""></a></figure>
                           <div class="lower-content">
                               <h3><a class="cursor_pointer_on" onclick="openWin_cd('8')" target="_blank">어머니 아들딸</a></h3>
                               <span class="designation"><br /></span>
                               <ul class="social-list">
                                   <li><a class="play_button"><i class="track-08 fas fa-play"></i></a></li>
                                   <li><a class="play_button"><i class="track-08 fas fa-pause"></i></a></li>
                                   <li><a class="cursor_pointer_on" onclick="openWin_cd('8')" target="_blank"><i class="far fa-file-alt"></i></a></li>
                               </ul>
                           </div>
                       </div>
                   </div>
               </div>
               <div class="col-lg-4 col-md-6 col-sm-12 block-column" style="padding:25px 30px;">
                   <div class="teachers-block-two wow fadeInUp" data-wow-delay="600ms" data-wow-duration="1500ms">
                       <div class="inner-box">
                           <figure class="image-box"><a href="#"><img src="<?php echo moshim_url('momo') ?>/assets/moshim/images/cd/23.jpg" alt=""></a></figure>
                           <div class="lower-content">
                               <h3><a class="cursor_pointer_on" onclick="openWin_cd('9')" target="_blank">어머니 손은 약손</a></h3>
                               <span class="designation"><br /></span>
                               <ul class="social-list">
                                   <li><a class="play_button"><i class="track-09 fas fa-play"></i></a></li>
                                   <li><a class="play_button"><i class="track-09 fas fa-pause"></i></a></li>
                                   <li><a class="cursor_pointer_on" onclick="openWin_cd('9')" target="_blank"><i class="far fa-file-alt"></i></a></li>
                               </ul>
                           </div>
                       </div>
                   </div>
               </div>
               <div class="col-lg-4 col-md-6 col-sm-12 block-column" style="padding:25px 30px;">
                   <div class="teachers-block-two wow fadeInUp" data-wow-delay="00ms" data-wow-duration="1500ms">
                       <div class="inner-box">
                           <figure class="image-box"><a href="#"><img src="<?php echo moshim_url('momo') ?>/assets/moshim/images/cd/25.jpg" alt=""></a></figure>
                           <div class="lower-content">
                               <h3><a class="cursor_pointer_on" onclick="openWin_cd('10')" target="_blank">해가 지고 달이 떠도</a></h3>
                               <span class="designation"><br /></span>
                               <ul class="social-list">
                                   <li><a class="play_button"><i class="track-10 fas fa-play"></i></a></li>
                                   <li><a class="play_button"><i class="track-10 fas fa-pause"></i></a></li>
                                   <li><a class="cursor_pointer_on" onclick="openWin_cd('10')" target="_blank"><i class="far fa-file-alt"></i></a></li>
                               </ul>
                           </div>
                       </div>
                   </div>
               </div>
               <div class="col-lg-4 col-md-6 col-sm-12 block-column" style="padding:25px 30px;">
                   <div class="teachers-block-two wow fadeInUp" data-wow-delay="300ms" data-wow-duration="1500ms">
                       <div class="inner-box">
                           <figure class="image-box"><a href="#"><img src="<?php echo moshim_url('momo') ?>/assets/moshim/images/cd/27.jpg" alt=""></a></figure>
                           <div class="lower-content">
                               <h3><a class="cursor_pointer_on" onclick="openWin_cd('11')" target="_blank">아름다운 나라 천일국</a></h3>
                               <span class="designation"><br /></span>
                               <ul class="social-list">
                                   <li><a class="play_button"><i class="track-11 fas fa-play"></i></a></li>
                                   <li><a class="play_button"><i class="track-11 fas fa-pause"></i></a></li>
                                   <li><a class="cursor_pointer_on" onclick="openWin_cd('11')" target="_blank"><i class="far fa-file-alt"></i></a></li>
                               </ul>
                           </div>
                       </div>
                   </div>
               </div>
               <div class="col-lg-4 col-md-6 col-sm-12 block-column" style="padding:25px 30px;">
                   <div class="teachers-block-two wow fadeInUp" data-wow-delay="600ms" data-wow-duration="1500ms">
                       <div class="inner-box">
                           <figure class="image-box"><a href="#"><img src="<?php echo moshim_url('momo') ?>/assets/moshim/images/cd/29.jpg" alt=""></a></figure>
                           <div class="lower-content">
                               <h3><a class="cursor_pointer_on" onclick="openWin_cd('12')" target="_blank">새로운 세상 우리 함께해요</a></h3>
                               <span class="designation"><br /></span>
                               <ul class="social-list">
                                   <li><a class="play_button"><i class="track-12 fas fa-play"></i></a></li>
                                   <li><a class="play_button"><i class="track-12 fas fa-pause"></i></a></li>
                                   <li><a class="cursor_pointer_on" onclick="openWin_cd('12')" target="_blank"><i class="far fa-file-alt"></i></a></li>
                               </ul>
                           </div>
                       </div>
                   </div>
               </div>
           </div>
           <!-- </div> -->
       </div>
   </section>
   <!-- our-teachers end -->
   <script>
       //cd 가사 해설 창 
       function openWin_cd(num) {
           window.open(`http://moshim.co.kr/pages/cd_detail/${num}`, '가사&해설', 'width=740, height=500, scrollbars=no');
           return false
       }
   </script>